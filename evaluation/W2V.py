# W2V class extends Evaluator class to evaluate accuracy of
# similarity via averaged Word2Vec vectors.
...
class W2V(Evaluator):
    ...
    def w2v_text_process(self, mess):
        """
        Returns list of the cleaned text in argument string mess,
        with stopwords, punctuation removed.
        """
        clean = [char if char not in string.punctuation else ' ' for char in mess]
        clean = ''.join(clean)
        clean = [word.lower() for word in clean.split() if word.lower() not in
                 stop_words.ENGLISH_STOP_WORDS]
        return clean

    # Create feature vector for each sentence by averaging all word embeddings in sentence
    ...
    def vectorize(self):
        ...
        self.model = Word2Vec(self.clean_corpus, size=300, window=5,

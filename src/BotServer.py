...
class BotServer:
    ...
    def match_query(self, query):
        """Prints most similar match in FAQ to user query, adding text concerning service requests if relevant."""
        index, similarity = self.tfidf_similarity(query)   
        
        response = self.faq.answer.iloc[index]
        
        if 'service request' in response.lower() or 'service request' in query.lower():
    ...
    def bot_dialog(self, request):
        """Given the argument POST request, parse it according to json or form data, print the locally-determined Dialogflow API intent, and return a json response based on sklearn matching within the FAQ."""
        if request.is_json:
